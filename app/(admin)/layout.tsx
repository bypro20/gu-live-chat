...
function AdminLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const [inboxRes, mailRes, healthRes] = await Promise.all([
          fetch('/api/admin/inbox-unread'),
          fetch('/api/admin/mail/unread-count'),
          fetch('/api/health'),
        ])
      ...
      <div className="h-screen flex items-center justify-center admin-shell admin-shell-v2 admin-text" data-admin-theme={adminTheme} style={{ background: 'var(--admin-bg)' }}>
        <div className="flex flex-col items-center gap-4">
          ...
          <div className="flex items-center gap-3 admin-text-muted text-sm">
            <svg className="animate-spin h-5 w-5" style={{ color: 'var(--admin-accent)' }} viewBox="0 0 24 24">
        ...
        >
          ...
          >
            ...
            >
              ...
              {inboxUnread > 0 && (
                <span className="absolute top-0.5 right-0.5 min-w-[1rem] h-4 px-0.5 text-white text-[9px] font-bold rounded-full flex items-center justify-center" style={{ background: 'var(--admin-accent)' }}>
