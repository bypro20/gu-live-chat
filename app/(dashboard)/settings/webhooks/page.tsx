  ...
  return (
    <div className="p-8 max-w-4xl">
      ...
      {showCreate && (
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-[#E5E0F0] dark:border-gray-700 p-6 mb-6">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {AVAILABLE_EVENTS.map((event) => (
                  <label key={event} className="flex items-center gap-2 p-2 bg-[#F5F3FF] dark:bg-gray-900 rounded-lg cursor-pointer hover:bg-[#EDE9FE] dark:hover:bg-gray-700 transition">
                    <input
                      type="checkbox"
          ...
          <div className="flex justify-end gap-3 mt-6">
            <button onClick={() => setShowCreate(false)} className="px-4 py-2.5 bg-[#EDE9FE] dark:bg-gray-700 text-[#4A2080] dark:text-gray-300 font-medium rounded-xl transition">
              İptal
            </button>
            ...
            {webhooks.map((webhook) => (
              <div key={webhook.id} className="p-4 flex items-center justify-between">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    {webhook.events.map((event) => (
                      <span key={event.id} className="px-2 py-0.5 bg-[#EDE9FE] dark:bg-gray-700 text-[#4A2080] dark:text-gray-400 text-xs rounded">
                        {event.event}
                      </span>
                    ))}
                  </div>
                ...
                <div className="flex items-center gap-3">
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${webhook.isActive ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 'bg-[#EDE9FE] text-[#4A2080] dark:bg-gray-700 dark:text-gray-400'}`}>
      ...
      <div className="mt-6 bg-[#6C3CE1]/10 dark:bg-purple-900/20 rounded-2xl p-4 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm font-medium text-[#4A2080] dark:text-purple-400">Webhook güvenliği</p>
          <p className="text-xs text-[#4A2080]/80 dark:text-purple-400/70 mt-0.5">Her webhook isteği, webhook sırrınız ile imzalanır. İstek doğrulamak için <code className="bg-[#6C3CE1]/10 dark:bg-purple-900/50 px-1 rounded">X-Gu-Signature</code> header değerini kullanın.</p>
        </div>
      </div>
    </div>
  )
