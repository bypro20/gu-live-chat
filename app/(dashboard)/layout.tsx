          ...
          {navGroups.map((group) => (
            <div key={group.title}>
              <div className="flex items-center gap-2 px-2.5 mb-1.5">
                <span className="text-[9px] font-semibold uppercase tracking-[0.12em]" style={{ color: 'var(--sidebar-foreground)' }}>{group.title}</span>
          ...
          {isAdmin && (
            <div>
              ...
              >
                ...
                <span className="flex-1 text-[13px] font-medium" style={{ color: '#FB7185' }}>Admin Paneli</span>
                <span className="px-1.5 py-0.5 text-[9px] font-bold text-white rounded-md shadow-sm" style={{ background: 'linear-gradient(135deg, #EF4444, #DC2626)' }}>ADMIN</span>
              </a>
            </div>
          )}
        </nav>
...
function SidebarLink({ href, icon, label, badge, active }: {
  ...
  return (
    <a
      ...
      className="flex items-center gap-2.5 px-3 py-[9px] rounded-lg transition-all duration-200 group relative"
      style={active
        ? { background: 'var(--sidebar-active)', color: 'var(--sidebar-foreground-active)' }
        : { color: 'var(--sidebar-foreground)' }}
      onMouseEnter={(e) => { if (!active) { e.currentTarget.style.background = 'var(--sidebar-hover)'; e.currentTarget.style.color = '#E4E3ED' } }}
      onMouseLeave={(e) => { if (!active) { e.currentTarget.style.background = ''; e.currentTarget.style.color = 'var(--sidebar-foreground)' } }}
    >
      {active && (
        <div className="absolute left-0 top-1/2 -translate-y-1/2 w-[3px] h-5 rounded-r-full transition-all duration-200" style={{ background: 'var(--sidebar-active-border)', boxShadow: '0 0 12px var(--primary-glow)' }} />
      )}
      <span className="shrink-0">{icons[icon]}</span>
      <span className="flex-1 text-[13px] font-medium tracking-tight">{label}</span>
      {badge && (
        <span className="px-1.5 py-0.5 text-[10px] font-bold text-white rounded-full min-w-[18px] text-center" style={{ background: 'linear-gradient(135deg, #6C3CE1, #8B5CF6)', boxShadow: '0 4px 12px -4px rgba(108,60,225,0.5)' }}>
          {badge}
        </span>
      )}
    </a>
