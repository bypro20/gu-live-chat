...
import { ADMIN_MODULES } from '@/lib/admin-navigation'
...
function PulseMetric({
  ...
  return (
    ...
    >
      ...
      <p className="text-2xl font-bold tabular-nums mt-1 admin-text">{value}</p>
    ...
    <div className="admin-command-center max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8 space-y-8">
      <section className="admin-command-hero">
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4 mb-6">
          ...
          <Link
            href="/admin/platform"
            className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl admin-btn-primary shrink-0"
          >
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-lg font-bold admin-text flex items-center gap-2">
              <Zap className="w-5 h-5 admin-text-muted" />
              Tüm Modüller
            </h2>
            ...
            return (
              <div key={mod.id} className="admin-module-card">
                <div className="flex items-start gap-3 mb-4">
                  <div className="p-2.5 rounded-xl admin-text-muted" style={{ background: 'var(--admin-bg-hover)' }}>
                  ...
                  {mod.links.map((link) => (
                    <li key={link.href + link.label}>
                      ...
                      >
                        ...
                        <ArrowRight className="w-3.5 h-3.5 admin-text-faint group-hover:admin-text-link shrink-0 transition-colors" />
                      </Link>
                    </li>
                  ))}
                </ul>
        ...
        <aside className="space-y-6">
          <div className="admin-panel-card">
            ...
            <div className="space-y-3">
              <MetricRow label="Aktif deneme" value={stats.trialFunnel.activeTrials} />
              <MetricRow label="48s içinde biten" value={stats.trialFunnel.expiringWithin48h} />
          ...
          <div className="admin-panel-card">
            <h3 className="text-sm font-bold admin-text flex items-center gap-2 mb-4">
              <TrendingUp className="w-4 h-4 admin-text-muted" />
              Plan Dağılımı
            </h3>
            <div className="space-y-3">
          ...
          <div className="admin-panel-card">
            <h3 className="text-sm font-bold admin-text flex items-center gap-2 mb-4">
              <Shield className="w-4 h-4 admin-text-muted" />
              Güvenlik Özeti
            </h3>
            <div className="space-y-3">
...
function MetricRow({
  ...
  const inner = (
    <div className="flex items-center justify-between text-sm">
      ...
      <span className="font-bold tabular-nums admin-text">
        {value}
      </span>
    </div>
  )
    ...
    <div className="admin-panel-card overflow-hidden p-0">
      <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: 'var(--admin-border)' }}>
        <h3 className="text-sm font-bold admin-text flex items-center gap-2">
          <Icon className="w-4 h-4 admin-text-muted" />
          {title}
        </h3>
  ...
  return (
    <div className="admin-panel-card">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`admin-status-badge ${allOk ? '' : 'admin-status-badge--alert'}`}>
...
function HealthRow({
  ...
  const statusClass = ok ? 'admin-text-muted' : warnWhenOff ? 'admin-text-muted' : 'admin-text-accent'
  return (
    <div className="flex items-center justify-between">
      <span className="flex items-center gap-2 admin-text-muted">
        <Icon className={`w-3.5 h-3.5 ${statusClass}`} />
        {label}
      </span>
