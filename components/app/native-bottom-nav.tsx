...
export function NativeBottomNav() {
  ...
  const tabs = [
    ...
    },
    {
      href: '/settings',
      label: nav.settings,
      match: (p: string) => p.startsWith('/settings') || p === '/analytics' || p === '/visitors',
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
  ...
  const mainMenu = shell.mainMenu

  return (
    <nav className="native-bottom-nav" aria-label={mainMenu}>
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className="native-tab-label">{tab.label}</span>
            </Link>
          )
        })}
      </div>
