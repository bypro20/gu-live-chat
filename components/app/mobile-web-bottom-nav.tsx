...
export function MobileWebBottomNav({
  ...
  const { nativeNav: nav, shell } = useDashboardI18n()
  ...
  const tabs: Tab[] = [
    ...
    },
    {
      href: '/settings',
      label: nav.settings,
      match: (p) => p.startsWith('/settings') || p === '/analytics' || p === '/visitors',
      icon: (
        <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
  ...
  return (
    <nav className="mobile-web-bottom-nav lg:hidden" aria-label={shell.mainMenu}>
      <div className="mobile-web-bottom-nav-inner">
            ...
            >
              <span className="relative mobile-web-tab-icon">
                ...
                {tab.badge != null && tab.badge > 0 && (
                  <span className="absolute -top-1 -right-2 min-w-[1rem] h-4 px-0.5 flex items-center justify-center text-[9px] font-bold bg-rose-500 text-white rounded-full">
        ...
        {onOpenMenu && (
          ...
          >
            <span className="mobile-web-tab-icon">
              <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
              </svg>
            </span>
            <span className="mobile-web-tab-label">{shell.menu}</span>
          </button>
        )}
      </div>
    </nav>
