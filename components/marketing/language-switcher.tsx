...
import { regionFlag, regionLabel } from '@/lib/regional-config'

const options: { locale: SiteLocale; label: string }[] = [
  { locale: 'tr', label: 'Türkçe' },
  { locale: 'en', label: 'English (worldwide)' },
]

export function LanguageSwitcher({ compact = false }: { compact?: boolean }) {
  const { locale, region, currency, setLocale } = useLocale()
  ...
  return (
    <div className={`flex items-center gap-2 ${compact ? '' : 'px-1'}`}>
      {!compact && (
        <span className="hidden sm:inline text-xs text-muted-foreground" title={label}>
          {flag}
        </span>
      )}
