#!/usr/bin/env python3
"""
Gu Live Chat — 60 sn İngilizce motion-graphics reklam (9:16).
Çıktı: ../../gulivechat-reklam-60s-en.mp4
"""
...
SCENES_DIR = ROOT / 'output' / 'scenes-en'
OUT_DIR = ROOT / 'output'
FINAL = ROOT.parent.parent / 'gulivechat-reklam-60s-en.mp4'
PREVIEW = ROOT.parent.parent / 'gulivechat-reklam-60s-en-izle.html'
...
SCENES_VOICE = [
    'Gu Live Chat. The all-in-one platform for modern customer conversations.',
    'Every visitor on your website deserves an instant, personal connection.',
    'Embed a beautiful live chat widget. Reply in seconds, right on your site.',
    'AI-powered auto-replies handle common questions, twenty-four seven.',
    'WhatsApp, email, and social channels — all in one unified inbox.',
    'Your team collaborates effortlessly. Never miss a lead again.',
    'Faster responses mean higher conversion and happier customers.',
    'Start your seven-day PRO trial free. No credit card required. Visit gulivechat dot com.',
]

SCENES_SUBTITLE = [
    'Gu Live Chat — Customer Conversations',
...
def _widget_ui(draw, y, t):
    ...
    draw.text((bx + 24, y + 22), 'Gu Live Chat  ·  Online', font=_font(26, True), fill=WHITE)
    if t > 0.2:
        e2 = _ease(min(1, (t - 0.2) * 2.5))
        draw.rounded_rectangle((bx + 20, y + 100, bx + bw - 90, y + 172), radius=16, fill=(24, 30, 52))
        draw.text((bx + 36, y + 124), 'Can I get pricing info?', font=_font(24), fill=WHITE)
    if t > 0.45:
        e3 = _ease(min(1, (t - 0.45) * 2.5))
        draw.rounded_rectangle((bx + 80, y + 192, bx + bw - 20, y + 276), radius=16, fill=ACCENT)
        draw.text((bx + 100, y + 216), 'Absolutely! Happy to help.', font=_font(24), fill=WHITE)
    if t > 0.65:
...
def _inbox_channels(draw, y, t):
    tags = ['Live Chat', 'WhatsApp', 'Email', 'Instagram']
...
def render_01(t, sub):
    ...
    _headline(draw, 340, 'Modern Customer', 56, WHITE, t)
    _headline(draw, 420, 'Conversations', 56, WHITE, max(0, t - 0.07))
    _headline(draw, 500, 'Made Simple', 64, TEAL, max(0, t - 0.16))
...
def render_02(t, sub):
    ...
    _headline(draw, 360, 'Every Visitor', 60, WHITE, t)
    _headline(draw, 450, 'Deserves Attention', 56, GOLD, max(0, t - 0.12))
    for i in range(6):
...
def render_03(t, sub):
    ...
    _headline(draw, 200, 'Live Chat Widget', 54, WHITE, t)
    _headline(draw, 268, 'Reply in Seconds', 40, ACCENT2, max(0, t - 0.1))
...
def render_04(t, sub):
    ...
    _headline(draw, 220, 'AI Auto-Reply', 58, WHITE, t)
    _headline(draw, 296, '24 / 7 Support', 44, TEAL, max(0, t - 0.1))
...
def render_06(t, sub):
    ...
    _headline(draw, 240, 'Your Team', 58, WHITE, t)
    _headline(draw, 320, 'Works Together', 58, ACCENT2, max(0, t - 0.1))
    _team_avatars(draw, 620, t)
    _feature_row(draw, 780, ['Shared inbox', 'Real-time notifications'], max(0, t - 0.2))
    return _subtitle_bar(img, sub)
...
def render_07(t, sub):
    ...
    _headline(draw, 240, 'Higher Conversion', 54, WHITE, t)
    _headline(draw, 320, 'Happier Customers', 54, GOLD, max(0, t - 0.1))
...
def render_08(t, sub):
    ...
    _headline(draw, 360, '7-Day PRO Trial', 54, WHITE, t)
    _headline(draw, 440, 'Free · No Card', 48, TEAL, max(0, t - 0.1))
...
def _ssml_en(text: str) -> str:
    t = text.strip()
    t = t.replace('gulivechat dot com', 'gulivechat dot com')
    t = t.replace('Gu Live Chat', '<prosody rate="-8%" pitch="+1Hz">Gu Live Chat</prosody>')
    t = re.sub(r'\.(\s+)', r'.<break time="380ms"/>\1', t)
    t = t.replace('—', '<break time="300ms"/>')
    return (
        '<speak version="1.0" xmlns="http://www.w3.org/2001/10/synthesis" '
        f'xmlns:mstts="https://www.w3.org/2001/mstts" xml:lang="en-US">'
        f'<voice name="{VOICE}"><prosody rate="{VOICE_RATE}" pitch="{VOICE_PITCH}">{t}</prosody></voice>'
        '</speak>'
    )
...
async def _tts(text: str, path: Path) -> None:
    import edge_tts
    await edge_tts.Communicate(_ssml_en(text), voice=VOICE).save(str(path))
...
async def _build_all() -> None:
    ...
    merged = OUT_DIR / 'reel-60s-en-merged.mp4'
    _concat(voiced, merged)
    music = OUT_DIR / 'reel-60s-en-music.mp3'
    _make_music(TARGET_TOTAL, music)
    _add_music(merged, music, FINAL, TARGET_TOTAL)

    PREVIEW.write_text(f'''<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/>
<meta name="viewport" content="width=device-width,initial-scale=1"/>
<title>Gu Live Chat — 60s Ad (English)</title>
...
<h2>Gu Live Chat — 60 Second Ad (English)</h2>
<p>8 scenes · Motion graphics · 9:16 vertical</p>
<video controls playsinline autoplay src="./gulivechat-reklam-60s-en.mp4"></video><br>
<a href="./gulivechat-reklam-60s-en.mp4" download>Download MP4</a>
<a href="./videolar.html">All videos</a>
...
def main() -> None:
    print('🏆 60s English motion-graphics ad (8 scenes)...')
